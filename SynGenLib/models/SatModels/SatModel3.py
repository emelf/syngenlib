...
class SaturationModel3: 
    ...
    def _objective(self, X, V_t, I_a, phi):
        ...
        sat_d = 1 / (1 + c_sat)
        sat_q = 1/(1 + self.md.X_aq_u/self.md.X_ad_u * c_sat)
        X_ad_sat = self.md.X_ad_u * sat_d
        # X_d_sat = X_ad_sat + self.md.X_l 
        X_aq_sat = self.md.X_aq_u * sat_q
        # X_q_sat = X_aq_sat + self.md.X_l

        f1 = delta - np.arctan(I_a*(X_q_sat*np.cos(phi)-(self.md.R_a*np.sin(phi)))/(V_t+(self.md.R_a*I_a*np.cos(phi))+X_q_sat*I_a*np.sin(phi)))
        f2 = c_sat - self.md.SG10 * psi_m**self.md.exp / psi_m
        f3 = X_d_sat - (X_ad_sat + self.md.X_l)
        f4 = X_q_sat - (X_aq_sat + self.md.X_l)
    ...
    def calc_ifd(self, V_t, I_a, phi) -> Tuple[float, float, float]: 
        ...
        delta_res, c_sat_res, X_d_sat, X_q_sat = root(self._objective, np.array([delta, c_sat, self.md.X_d_u, self.md.X_q_u]), args=(V_t, I_a, phi)).x
        X_ad_sat = X_d_sat - self.md.X_l
        print(f"X_d_sat = {X_d_sat:.3f}, X_q_sat = {X_q_sat:.3f}")
